...
echo -e "${BLUE}[BEGIN]${ENDCOLOR} Beginning MajorApi Worker development build process."
echo
...
echo -e "${BLUE}[BEGIN]${ENDCOLOR} Building the MajorApi Worker project."
php config/phing/bin/phing -Dbuild_settings_file=app/config/build.settings >/dev/null 2>&1

if [ ! -d "log" ]; then
    mkdir log
fi
echo -e "${GREEN}[FINISHED]${ENDCOLOR} Building the MajorApi Worker project."

echo
echo -e "${GREEN}####################################################################${ENDCOLOR}"
echo -e "${GREEN}# [SUCCESS] The MajorApi Worker project is ready to go!            #${ENDCOLOR}"
echo -e "${GREEN}####################################################################${ENDCOLOR}"
echo
